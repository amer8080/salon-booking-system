...
function analyzeError(error: any): ErrorInfo {
  ...
  const statusCode =
    error.status ||
    (error.message?.match(/\d{3}/) ? parseInt(error.message.match(/\d{3}/)[0]) : null);
...
export function logBookingError(error: any, _context?: ErrorContext) {
  const errorLog = {
    ...
    _context: {
      ...
      userAgent:
        (typeof window !== 'undefined' && _context?.userAgent) ||
  ...
  if (process.env.NODE_ENV === 'production') {
    ...
    try {
      // Example: window.gtag?.('event', 'exception', { description: errorLog.error.message })
    } catch (loggingError) {
...
export function createErrorBoundaryConfig(onError?: (error: Error, errorInfo: any) => void) {
  return {
