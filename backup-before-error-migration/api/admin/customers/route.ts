...
export async function GET(_request: NextRequest) {
  try {
    ...
    // Search filter
    if (search) {
      whereConditions.OR = [{ name: { contains: search } }, { phone: { contains: search } }];
    }
    ...
    const [totalCustomers, activeCustomers, vipCustomersCount, newCustomersThisMonth] =
      await Promise.all([
        prisma.customer.count(),
...
export async function POST(_request: NextRequest) {
  try {
    ...
    return NextResponse.json({ success: false, error: 'خطأ في إضافة العميل' }, { status: 500 });
  }
}
