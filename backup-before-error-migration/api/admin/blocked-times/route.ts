  ...
  try {
    ...
    const formattedBlockedTimes = blockedTimes.map((blocked) => ({
      ...
      startTime: blocked.startTime
        ? formatIstanbulDate(fromDatabaseTime(blocked.startTime), 'time')
        : null,
      endTime: blocked.endTime
        ? formatIstanbulDate(fromDatabaseTime(blocked.endTime), 'time')
        : null,
    ...
    return NextResponse.json({
      ...
      blockedTime: {
        ...
        startTime: newBlockedTime.startTime
          ? formatIstanbulDate(fromDatabaseTime(newBlockedTime.startTime), 'time')
          : null,
        endTime: newBlockedTime.endTime
          ? formatIstanbulDate(fromDatabaseTime(newBlockedTime.endTime), 'time')
          : null,
