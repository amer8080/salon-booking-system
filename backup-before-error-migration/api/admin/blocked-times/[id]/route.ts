...
export async function DELETE(
  ...
  try {
    ...
    if (isNaN(blockedTimeId)) {
      return NextResponse.json({ success: false, error: 'معرف الإقفال غير صحيح' }, { status: 400 });
    }
    ...
    if (!blockedTime) {
      return NextResponse.json({ success: false, error: 'الإقفال غير موجود' }, { status: 404 });
    }
