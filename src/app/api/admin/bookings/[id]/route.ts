  ...
  try {
    ...
    if (dateChanged || timeChanged) {
      if (!selectedDate || !selectedTime) {
        return NextResponse.json({ success: false, error: 'التاريخ والوقت مطلوبان لنقل الحجز' }, { status: 400 })
      }
    ...
    const successMessage = (dateChanged || timeChanged) ? 'تم نقل وتحديث الحجز بنجاح' : 'تم تحديث الحجز بنجاح'
