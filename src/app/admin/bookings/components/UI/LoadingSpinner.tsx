...
export default function LoadingSpinner({ 
  ...
  if (size === 'small') {
    return (
      <div className={`flex items-center space-x-2 rtl:space-x-reverse ${className}`}>
        {showIcon && (
          <div className={`${currentSize.container} bg-gradient-to-r from-pink-500 to-purple-600 rounded-full flex items-center justify-center animate-pulse`}>
            <Calendar className={`${currentSize.icon} text-white`} />
          </div>
        )}
        {message && (
          <p className={`text-gray-600 ${currentSize.text}`}>{message}</p>
        )}
      </div>
    )
  }
  ...
  return (
    <div className={`min-h-screen bg-gradient-to-br from-purple-50 via-blue-50 to-indigo-50 flex items-center justify-center ${className}`}>
      <div className="text-center">
        {showIcon && (
          <div className={`${currentSize.container} bg-gradient-to-r from-pink-500 to-purple-600 rounded-full flex items-center justify-center mx-auto mb-4 animate-pulse`}>
            <Calendar className={`${currentSize.icon} text-white`} />
          </div>
        )}
        {message && (
