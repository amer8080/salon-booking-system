...
export default function MonthView({
  ...
  const isDayBlocked = useCallback(
    (dateString: string) => {
      return blockedTimes.some(
        (blocked) =>
          blocked.startTime === null && blocked.endTime === null && blocked.date === dateString,
      );
    },
    [blockedTimes],
  );
  ...
  const handleDayClick = useCallback(
    (dayData: { date: string; day: number; isCurrentMonth: boolean }) => {
      ...
      <div className="grid grid-cols-7 gap-1 lg:gap-2" dir="rtl">
        {daysData.map((dayData, index) => {
          ...
          if (!dayData || !dayData.isCurrentMonth) {
            return (
              <div key={`empty-${index}`} className="h-10 lg:h-12 flex items-center justify-center">
                {dayData && <span className="text-xs text-gray-300">{dayData.day}</span>}
              </div>
            );
          }
          ...
          let circleClasses =
            'w-6 h-6 lg:w-8 lg:h-8 rounded-full flex items-center justify-center text-xs lg:text-sm font-medium transition-all duration-200 smooth-transition';
          ...
          if (isToday) {
            ...
            circleClasses += isPast ? ' text-gray-400' : ' text-gray-700 booking-active-available'; // أيام عادية مع تفاعل مخصص
          }
          ...
          return (
            ...
            >
              <div className={circleClasses}>{dayData.day}</div>
            </button>
          );
        })}
      </div>
          ...
          {blockedTimes.filter((b) => b.startTime === null && b.endTime === null).length > 0 && (
            <div className="flex items-center space-x-1 lg:space-x-2 rtl:space-x-reverse">
              ...
              <span className="text-gray-600">
                مقفلة (
                {blockedTimes.filter((b) => b.startTime === null && b.endTime === null).length})
              </span>
            </div>
          )}
        </div>
