...
export function useTimeSlots({
    ...
    async (date?: string): Promise<boolean> => {
      ...
      try {
        ...
        if (data.success) {
          ...
          if (
            state.selectedTime &&
            !timeSlots.find((slot) => slot.time === state.selectedTime && slot.isAvailable)
          ) {
  ...
  const bookedSlots = useMemo(() => state.slots.filter((slot) => slot.isBooked), [state.slots]);

  const blockedSlots = useMemo(() => state.slots.filter((slot) => slot.isBlocked), [state.slots]);

  const allSlots = state.slots;

  // Helper functions
  const getSlotByTime = useCallback(
    (time: string): TimeSlot | undefined => {
  ...
  const getPreviousAvailableTime = useCallback(
    (fromTime?: string): string | null => {
      const startTime = fromTime || state.selectedTime;
