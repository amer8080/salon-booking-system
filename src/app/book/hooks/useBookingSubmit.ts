...
export function useBookingSubmit({
  ...
  const submitBooking = useCallback(
    async (formData: BookingFormData, services: Service[]): Promise<boolean> => {
      ...
      try {
        ...
        const retrySubmit = createRetryWithBackoff(
          async () => {
  ...
  const openWhatsApp = useCallback(
    (formData: BookingFormData, services: Service[]) => {
  ...
  useEffect(() => {
    ...
    if ('connection' in navigator) {
      ...
      if (connection) {
        const updateSpeed = () => {
          const { effectiveType, downlink } = connection;
          setConnectionSpeed(effectiveType === '4g' && downlink > 1.5 ? 'fast' : 'slow');
        };
