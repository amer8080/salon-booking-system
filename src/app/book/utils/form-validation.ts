...
export function validateSelectedTime(
  ...
  if (date === today) {
    ...
    if (selectedTimeInMinutes <= currentTimeInMinutes + 30) {
      // إضافة 30 دقيقة buffer
      return {
        isValid: false,
        error: 'لا يمكن اختيار وقت مضى أو قريب جداً. يرجى اختيار وقت بعد 30 دقيقة على الأقل',
      };
    }
  }
 ...
 */
export function canProceedToNextStep(formData: BookingFormData): {
  canProceed: boolean;
  reason?: string;
} {
